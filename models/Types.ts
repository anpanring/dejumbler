...
const AlbumSchema = Item.discriminator<IAlbum>('Album', new mongoose.Schema({
...
const ArtistSchema = Item.discriminator<IArtist>('Artist', new mongoose.Schema({
    genres: [String], //string array for artist's genres
}, options));
// }

// Song inherits Item properties
const SongSchema = Item.discriminator<ISong>('Song', new mongoose.Schema({
    artist: { type: String, required: true },
    album: { type: String },
    lengthMins: { type: Number },
}, options));
...
const BookSchema = Item.discriminator<IBook>('Book', new mongoose.Schema({
...
const MovieSchema = Item.discriminator<IMovie>('Movie', new mongoose.Schema({
